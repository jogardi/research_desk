...
def generate_summary(text: str) -> str:
    ...
    # raise Exception('Testing error handling in generate_summary function.')
    # prompt = 'List the facts and points in the given text. \n\nText:' + text + 'Re-write the text in a clear manner in logical order. Summery: '
    messages = [
        { "role": "system", "content": "Use the following text as context when answering user queries:\n\n" + text },
        { "role": "user", "content": 'Generate a summary of the context text above.'},
    ]
...
if __name__ == "__main__":
    ...
    summary, error_response = generate_summary(text)
    if error_response is not None:
        print(f"Failed to generate summary: {error_response}")
    else:
        print(summary)
        print('-'*50)
        print(text)
