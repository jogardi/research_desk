...
def generate_queries(text: str) -> str:
    
    # Query the language model for suggested queries

    messages = [
        { "role": "system", "content": f"{hpc().PROMPT_SUGGEST_QUERIES}:\n\nProvided text:\n{text}" },
        { "role": "user", "content": "Generate a list of search queries related to the text."},
    ]
...
if __name__ == "__main__":
    ...
    queries, error_response = generate_answer(text)
    if error_response is not None:
